...
void Key_Init(void){ volatile uint32_t delay;
	...
	GPIO_PORTE_DEN_R |= 0x0F; //turns on PE0-3
	GPIO_PORTE_DIR_R &= ~0x0F; // sets PE0-3 as inputs
	GPIO_PORTF_DEN_R |= 0x02; //turns on digital reg for PF1
	GPIO_PORTF_DIR_R |= 0x02; // makes PF1 an output
}
// **************Key_In*********************
...
uint32_t Key_In(void){ 
    ...
    return 16; //will never get to here, just need a return statement 
}
